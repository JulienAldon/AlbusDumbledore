...
async def get_student_log(db: Session = Depends(get_db)):
    ...
    json_student_log = [{'id': elem.id, 'house': db.query(Student).filter_by(name=elem.student_name).first().house, 'name': elem.student_name, 'points': elem.student_points, 'reason': elem.reason} for elem in student_log]
